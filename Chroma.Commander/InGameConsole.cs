...
public class InGameConsole : DisposableResource
{
    ...
    public InGameConsole(Window window, int maxLines = 20, ICommandRegistry? registry = null, string? motd = null, int historySize = 20)
    {
        ...
        _target = new RenderTarget(
            window.Size.Width,
            (maxLines * _ttf.Height + 2) + (_ttf.Height + 2) + 4
        );
    ...
    public void Draw(RenderContext context)
    {
        ...
        context.RenderTo(_target, () =>
        {
            ...
            context.Line(
                new(0, _inputLine.Position.Y - 3),
                new(_target.Width, _inputLine.Position.Y - 3),
                ColorScheme.Border
            );
    ...
    private string ProcessCommand(string input)
    {
        var parsed = input.Trim().Split('"')
            ...
            .SelectMany(element => element).Cast<object>().ToArray();

        return _registry.Call((string)parsed.First(), parsed[1..parsed.Length]);
    }
